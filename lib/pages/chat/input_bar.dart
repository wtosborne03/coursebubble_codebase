...
import 'package:coursebubble/widgets/mxc_image.dart';
...
class InputBar extends StatelessWidget {
      ...
      for (final r in room.client.rooms) {
        ...
        if ((state != null &&
                ((state.content['alias'] is String &&
                        (state.content['alias'] as String)
                            .split(':')[0]
                            .toLowerCase()
                            .contains(roomSearch)) ||
                    (state.content['alt_aliases'] is List &&
                        (state.content['alt_aliases'] as List<dynamic>).any(
                          (l) =>
                              l is String &&
                              l
                                  .split(':')[0]
  ...
  Widget buildSuggestion(
    ...
    const size = 30.0;
    const padding = EdgeInsets.only(left: 8, top: 4, bottom: 4, right: 16);
    ...
    if (suggestion['type'] == 'user' || suggestion['type'] == 'room') {
      ...
      if (suggestion['type'] == 'user' && suggestion['displayname'] == 'admin')
        return SizedBox();
      return Row(children: [
        Container(
