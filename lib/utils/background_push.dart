...
import 'package:coursebubble/utils/push_helper.dart';
      ...
      (message) {
        ...
        pushHelper(
          PushNotification.fromJson(
            Map<String, dynamic>.from(message.data ?? message.data),
          ),
          client: client,
          l10n: l10n,
...
/*
    ...
    AwesomeNotifications().setListeners(
      ...
      if (action.payload != null)
        context!.vRouter.to('/rooms/' + action.payload!['room']!);
    });*/
  }
  ...
  Future<void> setupFirebase() async {
      ...
      try {
        if (kIsWeb) {
          _fcmToken = await FirebaseMessaging.instance.getToken(
              vapidKey:
                  "BIaJXi1dfL2iKO12pc8dL2eBT2t_u1cswVvC5Q8DtQefQ4Ci2C51yNlUJp2xAgzXklFpmqH12aRjDavIqFMOMG8");
        } else {
          _fcmToken = await FirebaseMessaging.instance.getToken();
        }
    ...
    FirebaseMessaging.onBackgroundMessage(bgHandle);
  }
  /*
...
Future<void> bgHandle(RemoteMessage message) async {
  String title = "";
  String body = "";
  String finalUrl = "";
  String senderString = "";
  ...
  print(await ClientManager.getClients(initialize: false));
  int notificationId = 0;
  try {
    Dio bdio = Dio();
    String roomId = message.data['room_id'];
    String eventId = message.data['event_id'];
    dynamic msgbody = (await bdio.get(
            'https://matrix.coursebubble.online/_matrix/client/v3/rooms/${roomId}/event/${eventId}?access_token=syt_YWRtaW4_czdHbDrGSHvCfdVQfMZv_0Yltfu'))
        .data;
    dynamic room = (await bdio.get(
            'https://matrix.coursebubble.online/_synapse/admin/v1/rooms/${roomId}?access_token=syt_YWRtaW4_czdHbDrGSHvCfdVQfMZv_0Yltfu'))
        .data;
    String senderId = msgbody['sender'];
    dynamic sender = (await bdio.get(
            'https://matrix.coursebubble.online/_matrix/client/v3/profile/${senderId}?access_token=syt_YWRtaW4_czdHbDrGSHvCfdVQfMZv_0Yltfu'))
        .data;
    String messagetext = msgbody['content']['body'];
    String senderName = sender['displayname'];
    String roomName = room['name'];

    String senderProfileUrl =
        'https://matrix.coursebubble.online/_matrix/media/r0/download/${sender['avatar_url'].toString().substring(6)}';

    title = roomName;
    body = senderName + ": " + messagetext;
    finalUrl = senderProfileUrl;
    print(finalUrl);
    finalRoomId = roomId;
  ...
  if (kIsWeb) {
    ...
    AwesomeNotifications().createNotification(
      content: NotificationContent(
        ...
        title: title,
        body: body,
        payload: {"room": finalRoomId},

        /*
        bigPicture: finalUrl.isEmpty ? null : finalUrl,
  ...
  /*
        await pushHelper(
          PushNotification(devices: [
            PushNotificationDevice(appId: message.data['d'], pushkey: '')
          ]),
          client: client,
          l10n: l10n,
          activeRoomId: router?.currentState?.pathParameters['roomid'],
